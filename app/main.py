...
def _build_home_context(request: Request, db: Session):
    # Performance Optimization: Use joinedload to eagerly fetch deals and best_store in a single query
    # rather than lazily loading them inside the template/logic, preventing N+1 queries.
    latest_run = (
        db.query(Run)
        .options(
...
def _build_admin_context(request: Request, db: Session, message: str = None, error: str = None):
    ...
    # Performance Optimization: Use joinedload to eagerly load the nested relationship
    # Run.published_stores -> PublishedSnapshotStore.dataset and Run.deals
    # This prevents an N+1 cascade when aggregating items_scraped_count across datasets.
    latest_run = (
        db.query(Run)
        .options(
